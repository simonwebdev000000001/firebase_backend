...
router.get('', function(req, res) {
  ...
  try {
    ...
    Devices.collection
      ...
      .get()
      .then((snapshot) => {
        res.json(new Devices.Schema({}).toList(snapshot));
...
router.post('', async function(req, res) {
  try {
    let item = new Devices.Schema(req.body);
    req.body.createdBy = req.user.uid;
    req.body.ipAddress = 'IP.adress';
    const createdItem = await Devices.collection.add(item.mapToCreate(req.body));
...
router.put('/:id', checkItem, async function(req, res) {
  try {
    ...
    await Devices.collection.doc(req.params.id).set(item.mapToUpdate(req.body), {
      merge: true,
    });
...
router.get('/:id', checkItem, async function(req, res) {
  try {
    ...
    item.id = req.params.id;
    res.json((new Devices.Schema(item).toJSON(true)));
...
router.delete('/:id', checkItem, async function(req, res) {
  try {
    ...
    await Devices.collection.doc(req.params.id).delete();
    res.json({
      message: 'Deteted',
    });
  } catch (error) {
...
async function checkItem(req, res, next) {
  try {
    let item = await Devices.collection.doc(req.params.id).get();
    if (!item.exists) {
      res.json({
        error: {
