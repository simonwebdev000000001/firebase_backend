...
router.get('', function(req, res) {
  ...
  try {
    ...
    Customer.collection
      ...
      .get()
      .then((snapshot) => {
        res.json(new Customer.Schema({}).toList(snapshot));
...
router.post('', async function(req, res) {
  try {
    let item = new Customer.Schema(req.body);
    req.body.createdBy = req.user.uid;
    const createdItem = await Customer.collection.add(item.mapToCreate(req.body));
...
router.put('/:id', checkItem, async function(req, res) {
  try {
    ...
    await Customer.collection.doc(req.params.id).set(item.mapToUpdate(req.body), {
      merge: true,
    });
...
router.get('/:id', checkItem,async function(req, res) {
  try {
      ...
      item.id = req.params.id;
      res.json((new Customer.Schema(item).toJSON(true)));
...
router.delete('/:id', checkItem,async function(req, res) {
  try {
    await Customer.collection.doc(req.params.id).delete();
    res.json({
      message: 'Deteted',
    });
  } catch (error) {
...
async function checkItem(req, res, next) {
  try {
    let item = await Customer.collection.doc(req.params.id).get();
    if (!item.exists) {
      res.json({
        error: {
